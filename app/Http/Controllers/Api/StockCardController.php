...
class StockCardController extends Controller
{
    ...
    public function getStockCard(Request $request)
    {
        $validator = Validator::make($request->all(), [
            "month_year"  => ['required', 'regex:/^(0[1-9]|1[0-2])-\d{4}$/']
        ]);
        ...
        $stock = DB::select("
            ...
            WHERE
                (a.procurement_date >= STR_TO_DATE(?, '%m-%Y') or a.sales_date >= STR_TO_DATE(?, '%m-%Y') or a.adjustment_date >= STR_TO_DATE(?, '%m-%Y') or a.usage_date >= STR_TO_DATE(?, '%m-%Y'))
